#!/usr/bin/env bash
# This script adds the provided public key to the web01 server

# Public key to add
PUBLIC_KEY="ssh-rsa "

# Connect to the server and add the key
ssh ubuntu@13.218.206.49 "mkdir -p ~/.ssh && echo \"$PUBLIC_KEY\" >> ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys" 